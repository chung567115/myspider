...
class BlogSpider:
    ...
    def process(self):
        # code = base64.b64encode('username:password'.encode())
    ...
    def get4bolg(self, url):
        for proxy in self.proxies:
            headers = {
                'User-Agent': random.choice(self.user_agents),
                "Accept-Encoding": "Gzip",
                # 'Proxy-Authorization': 'Basic ' + code.decode()
            }
            proxy_fix = proxy.strip()
            proxies = {'http': 'http://' + proxy_fix, 'https': 'https://' + proxy_fix}
            thread = Thread(target=self.request4blog, args=(url.strip(), headers, proxies, proxy_fix))
            thread.start()
            
    
    def request4blog(self, url, headers, proxies, proxy):
        try:
